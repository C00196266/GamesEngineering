	...
	for (std::vector<Entity*>::iterator i = entities.begin(), e = entities.end(); i != e; i++) {
		for (int j = 0; j < (*i)->getComponents().size(); j++) {
			if ((*i)->getComponents().at(j)->id == "health") {
				...
				if (static_cast<HealthComponent*>((*i)->getComponents().at(j))->getHealth() > 0) {
				...
				// assert stuff
				if (static_cast<PositionComponent*>((*i)->getComponents().at(j))->getPos().x < 1000 && static_cast<PositionComponent*>((*i)->getComponents().at(j))->getPos().y < 1000) {
